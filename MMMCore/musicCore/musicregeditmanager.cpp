...
bool MusicRegeditManager::currentNodeHasExist(const QString &key)
{
    ...
    state = (keyXSetting.value("Default").toString() == "MMMMusicPlayer." + key);
    ...
    state &= (fileExtsSetting.value("Progid").toString() == "MMMMusicPlayer." + key);
    ...
    state &= (fileExtsUserSetting.value("Progid").toString() == "MMMMusicPlayer." + key);
...
void MusicRegeditManager::createMusicRegedit(const QString &key, int index)
{
    ...
    if(keyX.isEmpty() || keyX != "MMMMusicPlayer." + key)
    {
        keyXSetting.setValue("Default", "MMMMusicPlayer." + key);
    }

    ////////////////////////////////////////////////////////
    const QString keyString = "HKEY_CLASSES_ROOT\\MMMMusicPlayer." + key;
    QSettings keySetting(keyString, QSettings::NativeFormat);
    keySetting.setValue("Default", key + QObject::tr("File"));

    const QString iconString = "HKEY_CLASSES_ROOT\\MMMMusicPlayer." + key + "\\DefaultIcon";
    ...
    const QString openString = "HKEY_CLASSES_ROOT\\MMMMusicPlayer." + key + "\\Shell\\Open";
    QSettings openSetting(openString, QSettings::NativeFormat);
    openSetting.setValue("Default", QObject::tr("user MMMMusicPlayer play"));

    const QString openComString = "HKEY_CLASSES_ROOT\\MMMMusicPlayer." + key + "\\Shell\\Open\\Command";
    ...
    const QString playListString = "HKEY_CLASSES_ROOT\\MMMMusicPlayer." + key + "\\Shell\\PlayList";
    QSettings playListSetting(playListString, QSettings::NativeFormat);
    playListSetting.setValue("Default", QObject::tr("add MMMMusicPlayer playList"));

    const QString playListComString = "HKEY_CLASSES_ROOT\\MMMMusicPlayer." + key + "\\Shell\\PlayList\\Command";
    ...
    const QString fileExtsString = "HKEY_CURRENT_USER\\Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\FileExts\\." + key;
    QSettings fileExtsSetting(fileExtsString, QSettings::NativeFormat);
    fileExtsSetting.setValue("Progid", "MMMMusicPlayer." + key);

    const QString fileExtsUserString = "HKEY_CURRENT_USER\\Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\FileExts\\." + key + "\\UserChoice";
    QSettings fileExtsUserSetting(fileExtsUserString, QSettings::NativeFormat);
    fileExtsUserSetting.setValue("Progid", "MMMMusicPlayer." + key);
